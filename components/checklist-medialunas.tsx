"use client";
// ═══════════════════════════════════════════════════════════════
// CONTROL VOLANTE — SABORES EXPRESS · TAPAS v1.0
// Proceso unificado P251 (Criollas) / P250 (Integrales) / P252 (Pastelitos)
// Trazabilidad fija de amasijos (path estable en Firebase)
// KPIs por etapa + KPI Auditoría Interna con gráfico
// ═══════════════════════════════════════════════════════════════
...
interface RTemp extends Base{tipo:"temperaturas";
...
interface RTapas extends Base{tipo:"tapas";
  variedad:TipoTapa;lote_amasijo:string;
  operario_amasado:string;operario_laminado:string;operario_cortadora:string;operario_envasado:string;
  // S0 — Fraccionado
  ...
  // S2 — Reposo cámara (solo criollas e integrales)
  num_tacho:string;fecha_ingreso_camara:string;hora_ingreso_camara:string;
  ...
  // S6 — Envasado
  tubos_por_cajon:string;unidades_por_tubo:string;cajones_etiquetados:boolean;
...
interface RBPM extends Base{tipo:"bpm";sector:string;operario:string;incumplimientos:string[];accion_tomada:string;responsable_sector:string;observaciones:string;}
interface RRecep extends Base{tipo:"recepcion";proveedor_id:string;proveedor_nombre:string;producto:string;remito_lote:string;cantidad_kg:string;vto:string;t_ingreso:string;estado_envase:string;rotulado_ok:boolean;fifo_ok:boolean;resultado:string;observaciones:string;}
interface RDesp extends Base{tipo:"despacho";local_destino:string;producto:string;lote:string;cantidad:string;t_despacho:string;etiquetado_ok:boolean;estado_embalaje:string;chofer:string;patente:string;observaciones:string;}
...
interface RLimp extends Base{tipo:"limpieza";sector:string;superficies_contacto:boolean;pisos_desagues:boolean;equipos:boolean;camaras:boolean;sanitizante:string;concentracion:string;responsable_limpieza:string;observaciones:string;}
...
interface RAuditoria{id:string;fecha:string;hora:string;responsable:string;turno:Turno;
  pct_recepcion:string;pct_amasado:string;pct_laminado:string;pct_cortadora:string;
  pct_envasado:string;pct_camara_pt:string;pct_bpm:string;pct_limpieza:string;
  pct_total:number;observaciones:string;acciones:string;}

interface TrazAmasijo{num_tacho:string;variedad:TipoTapa;lote_amasijo:string;etapas:TrazEtapa[];ultimo_update:string;}
interface TrazEtapa{etapa:string;fecha:string;hora:string;operario:string;turno:Turno;datos:Record<string,string>;}
...
// ── FIREBASE PATHS ────────────────────────────────────────────
function fsPath(mid:string,sem:number,fecha:string){return`cv_tapas_registros/${mid}/sem_${sem}/${fecha.replace(/-/g,"")}/items`;}
...
async function loadProveedores():Promise<Proveedor[]>{try{const s=await getDocs(collection(db,"cv_proveedores"));return s.docs.map(d=>d.data() as Proveedor);}catch{return[];}}
...
async function saveAuditoria(a:RAuditoria){await setDoc(doc(db,`cv_tapas_auditorias/${a.fecha}_${a.id}`),a as unknown as Record<string,unknown>);}
async function loadAuditorias(desde:string,hasta:string):Promise<RAuditoria[]>{
  try{const s=await getDocs(collection(db,"cv_tapas_auditorias"));
...
async function saveTrazAmasijo(t:TrazAmasijo){await setDoc(doc(db,`cv_tapas_amasijos/${t.num_tacho}`),t as unknown as Record<string,unknown>);}
async function loadTrazAmasijo(num:string):Promise<TrazAmasijo|null>{
  try{const d=await getDoc(doc(db,`cv_tapas_amasijos/${num}`));return d.exists()?d.data() as TrazAmasijo:null;}catch{return null;}}
async function loadTrazAmasijosSemana(desde:string,hasta:string):Promise<TrazAmasijo[]>{
  try{const s=await getDocs(collection(db,"cv_tapas_amasijos"));
    return s.docs.map(d=>d.data() as TrazAmasijo).filter(c=>{
      const ul=c.etapas[c.etapas.length-1]?.fecha||"";return ul>=desde&&ul<=hasta;
...
function calcHorasReposo(hi:string,di:string,hs:string,ds:string):number{
  ...
  try{const ini=new Date(`${di}T${hi}`);const sal=new Date(`${ds}T${hs}`);return Math.round((sal.getTime()-ini.getTime())/36000)/100;}catch{return 0;}
}

// Parámetros por variedad
const PARAMS={
  criollas:{pesoTapa:48,pesoTol:2,pesoBaston:{min:17.5,max:18},tAgua:{min:13,max:16},tMasa:{min:22,max:24},tubos:9,uPorTubo:48,cajones:192,label:"Tapas Criollas (P251)",proc:"P251",reposoMin:0},
  integrales:{pesoTapa:48,pesoTol:2,pesoBaston:{min:17.5,max:18},tAgua:{min:15,max:17},tMasa:{min:21,max:24},tubos:4,uPorTubo:48,cajones:192,label:"Tapas Integrales (P250)",proc:"P250",reposoMin:0},
  pastelitos:{pesoTapa:25,pesoTol:2,pesoBaston:{min:17.5,max:18},tAgua:{min:13,max:16},tMasa:{min:22,max:24},tubos:9,uPorTubo:48,cajones:432,label:"Tapas Pastelitos (P252)",proc:"P252",reposoMin:0},
};
...
function kpis(rs:Reg[]):KPI{
  let al=0,nc=0,dec=0,kg=0,binc=0,tap=0;const pt:Record<string,number>={};
  for(const r of rs){pt[r.tipo]=(pt[r.tipo]||0)+1;al+=cAl(r.alertas);if(r.tipo==="nc")nc++;if(r.tipo==="decomiso"){dec++;kg+=parseFloat((r as RDecom).cantidad_kg)||0;}if(r.tipo==="bpm")binc++;if(r.tipo==="tapas")tap++;}
...
function extraerAlertas(rs:Reg[]):AlertaItem[]{
  const labels:Record<string,{limite:string;tipo:string}>={
    t_agua_nc:{limite:"13–16 / 15–17°C",tipo:"T° agua chiller NC"},
    t_masa_nc:{limite:"22–24 / 21–24°C",tipo:"T° masa salida NC"},
    peso_nc:{limite:"48±2 / 25±2 g",tipo:"Peso tapa NC"},
    recupero_exc:{limite:"≤4 cajones",tipo:"Recupero excedido"},
    t_ambiente_nc:{limite:"≤21°C (integrales)",tipo:"T° ambiente NC"},
...
const UK="sv_tapas_usuarios_v1",PIN="1234";
...
function buildTxt(rs:Reg[],titulo:string,notas:Record<string,string>,elim:Set<string>):string{
  ...
  let t=`REPORTE — CONTROL VOLANTE TAPAS v1\nSabores Express\n${titulo}\nGenerado: ${new Date().toLocaleString("es-AR")}\n${"─".repeat(46)}\n\n`;
  ...
  for(const tr of TURNOS){const trs=vis.filter(r=>r.turno===tr.id);if(!trs.length)continue;t+=`\nTURNO ${tr.label.toUpperCase()}\n`;for(const r of trs){const m=MODS.find(x=>x.id===r.tipo);t+=`  [${r.hora}] ${m?.icon} ${m?.label}${cAl(r.alertas)>0?" ⚠":""} · ${r.responsable}\n`;
    if(r.tipo==="tapas"){const tp=r as RTapas;const pp=PARAMS[tp.variedad as keyof typeof PARAMS];t+=`    ${pp?.label||tp.variedad} | Lote: ${tp.lote_amasijo} | Prom.peso: ${tp.prom_peso}g | Desvío: ${tp.desvio_pct}%\n`;}
...
function Login({onLogin}:{onLogin:(u:Usuario)=>void}){
  const[n,sN]=useState("");const[t,sT]=useState<Turno>("TM");const[r,sR]=useState<Rol>("control_volante");const[pin,sPin]=useState("");const[err,sErr]=useState("");
  useEffect(()=>{try{const s=localStorage.getItem(UK);if(s){const u=JSON.parse(s) as Usuario;onLogin(u);}}catch{}},[]);
  function ok(){if(!n.trim()){sErr("Ingresá tu nombre");return;}if(r==="calidad"&&pin!==PIN){sErr("PIN incorrecto");return;}const u:Usuario={nombre:n.trim(),rol:r,turno:t};try{localStorage.setItem(UK,JSON.stringify(u));}catch{}onLogin(u);}
  return<div className="min-h-screen bg-gray-50 flex items-center justify-center p-4"><div className="bg-white rounded-2xl border border-gray-200 p-6 w-full max-w-sm flex flex-col gap-4">
    <div><p className="text-xs text-gray-400 uppercase tracking-wide">Sabores Express</p><p className="text-xl font-bold text-gray-800 mt-0.5">Control Volante 🫓</p><p className="text-xs text-gray-400 mt-0.5">Tapas — P251 / P250 / P252</p></div>
    <div className="flex flex-col gap-3">
      <Txt label="Tu nombre" value={n} onChange={sN} ph="Nombre y apellido"/>
      <div className="flex flex-col gap-0.5"><label className="text-xs text-gray-500">Turno</label><div className="flex gap-2">{TURNOS.map(x=><button key={x.id} onClick={()=>sT(x.id)} className={cn("flex-1 h-10 rounded-lg border text-sm font-medium",t===x.id?"bg-blue-500 border-blue-500 text-white":"border-gray-200 text-gray-600")}>{x.label}</button>)}</div></div>
      <div className="flex flex-col gap-0.5"><label className="text-xs text-gray-500">Rol</label><div className="flex gap-2">{([{id:"control_volante",l:"Control Volante"},{id:"calidad",l:"Calidad"}] as const).map(x=><button key={x.id} onClick={()=>sR(x.id)} className={cn("flex-1 h-10 rounded-lg border text-sm font-medium",r===x.id?"bg-blue-500 border-blue-500 text-white":"border-gray-200 text-gray-600")}>{x.l}</button>)}</div></div>
...
function FTemp({u,onSave,onCancel}:{u:Usuario;onSave:(r:Reg)=>void;onCancel:()=>void}){
  ...
  async function sv(){sG(true);onSave({id:gid("tmp"),tipo:"temperaturas",turno:u.turno,responsable:u.nombre,fecha:hoy(),hora:ahora(),timestamp:new Date().toISOString(),alertas:{t_camara_masas_nc:aCamaraMasas,t_ambiente_nc:aAmbiente,t_camara_pt_nc:aCamaraPT},...d} as unknown as Reg);sG(false);}
  return<FW titulo="🌡️ Temperaturas — Cámaras" sub="PC · Registro único de cámaras" onCancel={onCancel} onSave={sv} g={g} ch={<>
    <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-xs text-blue-700 leading-relaxed">Registrá una sola vez por turno. T° de proceso (masa, cortadora) van en <b>🫓 Control Tapas</b>.</div>
    ...
    <Num label="T° ambiente área de producción (°C)" spec="PC — ≤21°C (crítico para integrales)" value={d.t_ambiente} onChange={v=>sD(p=>({...p,t_ambiente:v}))} al={aAmbiente}/>
    {aAmbiente&&<div className="bg-amber-50 border border-amber-200 rounded-lg p-2 text-xs text-amber-700">⚠ T° ambiente elevada — impacta en masa de tapas integrales (límite 21°C)</div>}
...
function FTapas({u,onSave,onCancel}:{u:Usuario;onSave:(r:Reg)=>void;onCancel:()=>void}){
  const[d,sD]=useState({
    ...
    // S6 — Envasado
    tubos_por_cajon:"",unidades_por_tubo:"",cajones_etiquetados:false,
  ...
  const pesoObj=p?.pesoTapa||48;const pesoTol=p?.pesoTol||2;
  ...
  function setMuestra(i:number,v:string){sD(prev=>{const a=[...prev.muestras_peso];a[i]=v;return{...prev,muestras_peso:a};});}

  async function sv(){
    sG(true);
    if(d.num_tacho){
      await agregarEtapaAmasijo(d.num_tacho,{etapa:"amasado",fecha:d.fecha_ingreso_camara,hora:d.hora_ingreso_camara||ahora(),operario:u.nombre,turno:u.turno,datos:{t_agua:d.t_agua_chiller,t_masa:d.t_masa_salida,tiempo:d.tiempo_amasado,lote:d.lote_amasijo}},d.variedad,d.lote_amasijo);
      if(d.hora_salida_camara){
        await agregarEtapaAmasijo(d.num_tacho,{etapa:"laminado",fecha:d.fecha_salida_camara,hora:d.hora_salida_camara,operario:u.nombre,turno:u.turno,datos:{virgen_sobre_recupero:d.virgen_sobre_recupero?"ok":"nc",textura:d.textura_masa_ok?"ok":"nc"}},d.variedad,d.lote_amasijo);
      }
      await agregarEtapaAmasijo(d.num_tacho,{etapa:"cortadora",fecha:hoy(),hora:ahora(),operario:u.nombre,turno:u.turno,datos:{prom_peso:`${prom}g`,desvio:`${dv}%`,t_cortadora:d.t_cortadora}},d.variedad,d.lote_amasijo);
      await agregarEtapaAmasijo(d.num_tacho,{etapa:"envasado",fecha:hoy(),hora:ahora(),operario:u.nombre,turno:u.turno,datos:{tubos:d.tubos_por_cajon,cajones_ok:d.cajones_etiquetados?"ok":"nc",fifo:d.fifo_ok?"ok":"nc"}},d.variedad,d.lote_amasijo);
    }
    onSave({id:gid("tap"),tipo:"tapas",turno:u.turno,responsable:u.nombre,fecha:hoy(),hora:ahora(),timestamp:new Date().toISOString(),
  ...
  return<FW titulo="🫓 Control Tapas" sub="P251 Criollas / P250 Integrales / P252 Pastelitos" onCancel={onCancel} onSave={sv} g={g} ch={<>

    {/* ── CABECERA ─────────────────────────────────────────── */}
    <div className="flex gap-2">
      {(["criollas","integrales","pastelitos"] as const).map(x=><button key={x} onClick={()=>sD(p=>({...p,variedad:x}))} className={cn("flex-1 py-3 rounded-xl text-xs font-bold border-2",d.variedad===x?"border-amber-500 bg-amber-50 text-amber-700":"border-gray-200 bg-white text-gray-500")}>
        {x==="criollas"?"🫓 Criollas":x==="integrales"?"🌾 Integrales":"🟡 Pastelitos"}
      </button>)}
    </div>
    {p&&<div className="bg-amber-50 border border-amber-200 rounded-xl p-2.5 text-[10px] text-amber-700 leading-relaxed">
      <b>{p.label}</b> · Procedimiento {p.proc} · Peso objetivo: {pesoObj}g ±{pesoTol}g · {p.cajones} und/cajón
    </div>}
    ...
    <div className="rounded-2xl border-2 border-gray-200 overflow-hidden">
      <div className="bg-gray-100 px-4 py-2 flex items-center justify-between">
        ...
        <span className="text-[10px] text-gray-400">§2 todos los procedimientos</span>
      </div>
      <div className="p-4 flex flex-col gap-3 bg-white">
        <div className="text-[10px] text-gray-400 leading-relaxed">Dosificar aditivos (sorbato, propionato, sal, almidón, gluten), fraccionar grasa y margarina. Rotular con fecha de elaboración.</div>
    ...
    <div className="rounded-2xl border-2 border-indigo-200 overflow-hidden">
      ...
      <div className="p-4 flex flex-col gap-3 bg-white">
        {p&&<div className="text-[10px] text-indigo-400">Agua chiller: {p.tAgua.min}–{p.tAgua.max}°C · T° masa salida: {p.tMasa.min}–{p.tMasa.max}°C · Bastón: {p.pesoBaston.min}–{p.pesoBaston.max}kg</div>}
        <Num label="T° agua chiller (°C)" spec={p?`PC — ${p.tAgua.min}°C a ${p.tAgua.max}°C`:""} value={d.t_agua_chiller} onChange={v=>sD(p=>({...p,t_agua_chiller:v}))} al={!!aAgua}/>
        ...
        <Num label="T° masa al salir amasadora (°C)" spec={p?`PC — ${p.tMasa.min}°C a ${p.tMasa.max}°C`:""} value={d.t_masa_salida} onChange={v=>sD(p=>({...p,t_masa_salida:v}))} al={!!aTMasa}/>
        {aTMasa&&<div className="bg-red-50 border border-red-200 rounded-xl p-2 text-xs text-red-700">⚠ T° masa NC — fraccionar en tachos y reposar hasta que baje a rango. Documentar acción abajo.</div>}
        <Num label="Peso del bastón (kg)" spec="PC — 17.5 a 18 kg (ideal 18 kg)" value={d.peso_baston} onChange={v=>sD(p=>({...p,peso_baston:v}))} al={d.peso_baston!==""&&(parseFloat(d.peso_baston)<17.5||parseFloat(d.peso_baston)>18)}/>
        <Num label="Tiempo total de amasado (min)" spec="PC — ~8 min (más 1.5 min inicial)" value={d.tiempo_amasado} onChange={v=>sD(p=>({...p,tiempo_amasado:v}))}/>

        {/* Trazabilidad — Ingreso a cámara (solo criollas e integrales) */}
        {(d.variedad==="criollas"||d.variedad==="integrales")&&<div className="bg-cyan-50 border border-cyan-200 rounded-xl p-3 flex flex-col gap-2.5">
          ...
          {d.num_tacho&&d.hora_ingreso_camara&&<div className="bg-cyan-100 rounded-lg px-3 py-1.5 text-xs text-cyan-800 font-medium">Tacho {d.num_tacho} · Ingresó {fd(d.fecha_ingreso_camara)} a las {d.hora_ingreso_camara}h</div>}
        </div>}
      </div>
    </div>
    ...
    {/* ══════════════════════════════════════════════════════
        S2 — LAMINADO
        PC: textura · color · masa virgen sobre recupero
    ══════════════════════════════════════════════════════ */}
    <div className="rounded-2xl border-2 border-violet-200 overflow-hidden">
      ...
      <div className="p-4 flex flex-col gap-3 bg-white">
        {(d.variedad==="criollas"||d.variedad==="integrales")&&<div className="bg-violet-50 border border-violet-200 rounded-xl p-3 flex flex-col gap-2.5">
          ...
          {horasReposo>0&&<div className={cn("rounded-xl px-3 py-2 text-sm font-bold text-center border-2",horasReposo>=12?"border-blue-300 bg-blue-50 text-blue-700":"border-green-300 bg-green-50 text-green-700")}>
        ...
        <Chk label="✓ Textura y color de masa uniforme" value={d.textura_masa_ok} onChange={v=>sD(p=>({...p,textura_masa_ok:v}))}/>
        ...
        {d.variedad!=="pastelitos"
          ...
          :<div className="bg-amber-50 border border-amber-200 rounded-lg p-2 text-xs text-amber-700">ℹ️ Pastelitos: en laminadora se usa <b>solo masa recupero</b> (no se agrega virgen)</div>}
        {!d.virgen_sobre_recupero&&d.variedad!=="pastelitos"&&<div className="bg-red-50 border border-red-200 rounded-lg p-2 text-xs text-red-700">⚠ PC — Verificar orden de amasijos: virgen arriba, recupero abajo</div>}
        <div className="text-[10px] text-gray-400">Laminado en 2 vueltas (criollas y pastelitos) / 3 vueltas (integrales). Verificar limpieza de maquinaria.</div>
      </div>
    </div>

    {/* ══════════════════════════════════════════════════════
        S3 — CORTADORA + PESOS
        PC: peso 48±2g (criollas/integrales) / 25±2g (pastelitos)
            temperatura 18°C
    ══════════════════════════════════════════════════════ */}
    <div className="rounded-2xl border-2 border-orange-200 overflow-hidden">
      ...
      <div className="p-4 flex flex-col gap-3 bg-white">
        ...
        <Num label="T° cortadora / ambiente (°C)" spec="PC — 16°C a 20°C" value={d.t_cortadora} onChange={v=>sD(p=>({...p,t_cortadora:v}))} al={aTCort}/>
        ...
        <div className="bg-orange-50 border border-orange-200 rounded-xl p-3">
          ...
          <div className="grid grid-cols-5 gap-1.5 mb-3">
            {d.muestras_peso.map((v,i)=><div key={i} className="flex flex-col gap-0.5">
              <span className="text-[9px] text-gray-400 text-center">{i+1}</span>
              <input type="number" inputMode="decimal" value={v} onChange={e=>setMuestra(i,e.target.value)} placeholder="—" className={cn("h-10 rounded-lg border text-center text-xs font-mono",v!==""&&Math.abs(parseFloat(v)-pesoObj)>pesoTol?"border-red-400 bg-red-50 text-red-700":"border-gray-200 bg-white")}/>
            </div>)}
          </div>
          {prom>0&&<div className={cn("rounded-xl p-3 flex items-center justify-between border-2 mb-2",aPeso?"border-red-400 bg-red-50":"border-green-400 bg-green-50")}>
            <div><div className={cn("text-sm font-bold",aPeso?"text-red-700":"text-green-700")}>{aPeso?"⚠ Peso fuera de rango":"✓ Peso en rango"}</div><div className="text-[10px] text-gray-500">Promedio: {prom}g · Objetivo: {pesoObj}g ±{pesoTol}g</div></div>
    ...
    {/* ══════════════════════════════════════════════════════
        ...
        PC: ≤4 cajones de recupero al cierre
    ══════════════════════════════════════════════════════ */}
    <div className="rounded-2xl border-2 border-yellow-200 overflow-hidden">
      ...
      <div className="p-4 flex flex-col gap-3 bg-white">
        ...
        {d.variedad==="integrales"&&<div className="bg-yellow-50 border border-yellow-200 rounded-lg p-2 text-xs text-yellow-700">ℹ️ Integrales: el último recorte del turno se <b>pesa y decomisa</b> (no se guarda para el turno siguiente).</div>}
        <Num label="Cajones de recupero al cierre de turno" spec="PC — Máximo 4 cajones" value={d.recupero_cajones} onChange={v=>sD(p=>({...p,recupero_cajones:v}))} al={aRecupero}/>
    ...
    <div className="rounded-2xl border-2 border-teal-200 overflow-hidden">
      ...
      <div className="p-4 flex flex-col gap-3 bg-white">
        {p&&<div className="text-[10px] text-teal-500 mb-1">{p.tubos} tubos x {p.uPorTubo} unidades = {p.cajones} unid/cajón · 32 cajones/carro</div>}
        <Txt label="Operario envasado" value={d.operario_envasado} onChange={v=>sD(p=>({...p,operario_envasado:v}))} ph="Nombre y apellido"/>
        <div className="grid grid-cols-2 gap-2">
          <Num label="Tubos por cajón" spec={p?`${p.tubos} tubos/cajón`:""} value={d.tubos_por_cajon} onChange={v=>sD(p=>({...p,tubos_por_cajon:v}))} al={!!p&&d.tubos_por_cajon!==""&&parseInt(d.tubos_por_cajon)!==p.tubos}/>
          <Num label="Unidades por tubo" spec="48 und/tubo" value={d.unidades_por_tubo} onChange={v=>sD(p=>({...p,unidades_por_tubo:v}))} al={d.unidades_por_tubo!==""&&parseInt(d.unidades_por_tubo)!==48}/>
        </div>
        <Chk label="✓ Columna de tapas uniforme en el tubo" value={d.cajones_etiquetados} onChange={v=>sD(p=>({...p,cajones_etiquetados:v}))}/>
        <Chk label="✓ Bolsa sin aberturas y limpia" value={d.bolsa_integra_ok} onChange={v=>sD(p=>({...p,bolsa_integra_ok:v}))}/>
    ...
    <div className="rounded-2xl border-2 border-blue-200 overflow-hidden">
      ...
      <div className="p-4 flex flex-col gap-3 bg-white">
        <div className="text-[10px] text-blue-400">FIFO estricto: lo más antiguo cerca de la puerta de salida. 32 cajones por carro. Pallets: 64 cajones (8 columnas de 7 + 1 columna de 8).</div>
    ...
    <div className="rounded-2xl border-2 border-purple-200 overflow-hidden">
      ...
      <div className="p-4 flex flex-col gap-2 bg-white">
        <Chk label="✓ Forma correcta — circular uniforme (criollas/integrales) / rectangular (pastelitos)" value={d.forma_ok} onChange={v=>sD(p=>({...p,forma_ok:v}))}/>
        ...
        <TA label="Obs. sensorial" value={d.sensorial_obs} onChange={v=>sD(p=>({...p,sensorial_obs:v}))} ph="Desvíos de forma, color, textura, presencia de cuerpos extraños…"/>
      </div>
    </div>
...
function FBPM({u,onSave,onCancel}:{u:Usuario;onSave:(r:Reg)=>void;onCancel:()=>void}){
  ...
  return<FW titulo="👤 BPM — Incumplimiento" sub="Registrar operario y desvío" onCancel={onCancel} onSave={sv} g={g} ch={<>
    ...
    <Sel label="Sector" value={d.sector} onChange={v=>sD(p=>({...p,sector:v}))} opts={[{v:"amasado",l:"Amasado"},{v:"laminado",l:"Laminado"},{v:"cortadora",l:"Cortadora"},{v:"sobadora",l:"Sobadora"},{v:"envasado",l:"Envasado"},{v:"camara_pt",l:"Cámara PT"},{v:"recepcion_mp",l:"Recepción MP"}]}/>
...
function FRecep({u,onSave,onCancel}:{u:Usuario;onSave:(r:Reg)=>void;onCancel:()=>void}){
  ...
  async function sv(){sG(true);onSave({id:gid("rec"),tipo:"recepcion",turno:u.turno,responsable:u.nombre,fecha:hoy(),hora:ahora(),timestamp:new Date().toISOString(),alertas:{t_ingreso:at,rechazado:d.resultado==="rechazado"},...d} as RRecep);sG(false);}
  function selProv(id:string){const p=provs.find(x=>x.id===id);sD(prev=>({...prev,proveedor_id:id,proveedor_nombre:p?.nombre||""}));}
  return<FW titulo="🚚 Recepción MP" sub="Control de materia prima" onCancel={onCancel} onSave={sv} g={g} ch={<>
    ...
    <Sel label="Producto" value={d.producto} onChange={v=>sD(p=>({...p,producto:v}))} opts={[{v:"harina_0000",l:"Harina 0000"},{v:"harina_000",l:"Harina 000 (28% gluten)"},{v:"margarina",l:"Margarina Tapera"},{v:"grasa",l:"Grasa bovina"},{v:"almidon",l:"Almidón de maíz"},{v:"gluten",l:"Gluten"},{v:"sal",l:"Sal"},{v:"propionato",l:"Propionato de calcio"},{v:"sorbato",l:"Sorbato de potasio"},{v:"hielo",l:"Hielo en escama"},{v:"otro",l:"Otro"}]}/>
...
function FDesp({u,onSave,onCancel}:{u:Usuario;onSave:(r:Reg)=>void;onCancel:()=>void}){
  const[d,sD]=useState({local_destino:"",producto:"",lote:"",cantidad:"",t_despacho:"",etiquetado_ok:false,estado_embalaje:"",chofer:"",patente:"",observaciones:"",fotos:[] as FotoMeta[]});const[g,sG]=useState(false);
  async function sv(){sG(true);onSave({id:gid("dsp"),tipo:"despacho",turno:u.turno,responsable:u.nombre,fecha:hoy(),hora:ahora(),timestamp:new Date().toISOString(),alertas:{sin_etiqueta:!d.etiquetado_ok},...d} as RDesp);sG(false);}
  return<FW titulo="📦 Despacho" sub="Control de PT saliente" onCancel={onCancel} onSave={sv} g={g} ch={<>
    ...
    <Sel label="Producto" value={d.producto} onChange={v=>sD(p=>({...p,producto:v}))} opts={[{v:"criollas",l:"Tapas Criollas"},{v:"integrales",l:"Tapas Integrales"},{v:"pastelitos",l:"Tapas Pastelitos"}]}/>
...
function FNC({u,onSave,onCancel}:{u:Usuario;onSave:(r:Reg)=>void;onCancel:()=>void}){
  ...
  return<FW titulo="⚠️ No Conformidad" sub="Desvíos de proceso o producto" onCancel={onCancel} onSave={sv} g={g} ch={<>
    ...
    <Sel label="Tipo" value={d.tipo_nc} onChange={v=>sD(p=>({...p,tipo_nc:v}))} opts={[{v:"peso",l:"Peso fuera de rango"},{v:"temperatura",l:"Temperatura NC"},{v:"textura",l:"Textura / forma"},{v:"etiqueta",l:"Error etiquetado"},{v:"fifo",l:"FIFO no cumplido"},{v:"bpm",l:"BPM"},{v:"recupero",l:"Recupero excedido"},{v:"proveedor",l:"Proveedor / MP"},{v:"infraestructura",l:"Infraestructura/maquinaria"},{v:"otro",l:"Otro"}]}/>
    <TA label="Descripción del desvío" value={d.descripcion} onChange={v=>sD(p=>({...p,descripcion:v}))} ph="Qué, dónde, cuándo. Ej: Tapa criollas con peso 52g al salir de cortadora (límite 48±2g)"/>
    <Txt label="Lote afectado" value={d.lote_afectado} onChange={v=>sD(p=>({...p,lote_afectado:v}))}/>
    <Sel label="Causa raíz (5 porqués)" value={d.causa_raiz} onChange={v=>sD(p=>({...p,causa_raiz:v}))} opts={[{v:"humano",l:"Factor humano"},{v:"equipo",l:"Equipo / calibración"},{v:"metodo",l:"Método / procedimiento"},{v:"insumo",l:"Materia prima"},{v:"ambiente",l:"Temperatura ambiente"}]}/>
...
function FDecom({u,onSave,onCancel}:{u:Usuario;onSave:(r:Reg)=>void;onCancel:()=>void}){
  ...
  return<FW titulo="🗑️ Decomiso" sub="HACCP obligatorio" onCancel={onCancel} onSave={sv} g={g} ch={<>
    ...
    <Sel label="Producto" value={d.producto} onChange={v=>sD(p=>({...p,producto:v}))} opts={[{v:"criollas",l:"Tapas Criollas"},{v:"integrales",l:"Tapas Integrales"},{v:"pastelitos",l:"Tapas Pastelitos"},{v:"mp",l:"Materia Prima"}]}/>
    <Txt label="Lote" value={d.lote} onChange={v=>sD(p=>({...p,lote:v}))}/>
    <Num label="Cantidad (kg)" value={d.cantidad_kg} onChange={v=>sD(p=>({...p,cantidad_kg:v}))}/>
    <Sel label="Motivo" value={d.motivo} onChange={v=>sD(p=>({...p,motivo:v}))} opts={[{v:"vencido",l:"Vencido"},{v:"peso_nc",l:"Peso NC"},{v:"textura",l:"Textura/forma NC"},{v:"contaminacion",l:"Contaminación"},{v:"rotulado",l:"Error rotulado"},{v:"recupero_ex",l:"Recupero excedido (integrales)"},{v:"otro",l:"Otro"}]}/>
    <Sel label="Etapa de detección" value={d.etapa_deteccion} onChange={v=>sD(p=>({...p,etapa_deteccion:v}))} opts={[{v:"mp",l:"Recepción MP"},{v:"amasado",l:"Amasado"},{v:"laminado",l:"Laminado"},{v:"cortadora",l:"Cortadora"},{v:"envasado",l:"Envasado"},{v:"pt",l:"Cámara PT"},{v:"despacho",l:"Despacho"}]}/>
    <Sel label="Destino" value={d.destino} onChange={v=>sD(p=>({...p,destino:v}))} opts={[{v:"destruccion",l:"Destrucción"},{v:"devolucion",l:"Devolución a proveedor"},{v:"reproceso",l:"Reproceso"}]}/>
    <Fotos fotos={d.fotos} onAdd={f=>sD(p=>({...p,fotos:[...p.fotos,f]}))} onRemove={id=>sD(p=>({...p,fotos:p.fotos.filter(f=>f.id!==id)}))}/>
    <TA label="Observaciones" value={d.observaciones} onChange={v=>sD(p=>({...p,observaciones:v}))}/>
  </>}/>;
}
...
function FLimp({u,onSave,onCancel}:{u:Usuario;onSave:(r:Reg)=>void;onCancel:()=>void}){
  const[d,sD]=useState({sector:"",superficies_contacto:false,pisos_desagues:false,equipos:false,camaras:false,sanitizante:"",concentracion:"",responsable_limpieza:"",observaciones:"",fotos:[] as FotoMeta[]});const[g,sG]=useState(false);
  ...
  return<FW titulo="🧹 Limpieza POES" sub="POES/BPM" onCancel={onCancel} onSave={sv} g={g} ch={<>
    <Sel label="Sector" value={d.sector} onChange={v=>sD(p=>({...p,sector:v}))} opts={[{v:"amasado",l:"Amasadora"},{v:"laminado",l:"Laminadora"},{v:"cortadora",l:"Cortadora"},{v:"sobadora",l:"Sobadora"},{v:"envasado",l:"Envasado"},{v:"camara_masas",l:"Cámara de masas"},{v:"camara_pt",l:"Cámara PT"},{v:"sanitarios",l:"Sanitarios"}]}/>
    <div className="flex items-center justify-between"><div className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Verificación</div><div className={cn("text-sm font-bold",pc===100?"text-green-600":"text-amber-600")}>{pc}%</div></div>
...
function BDProveedores({onBack}:{onBack:()=>void}){
  ...
  const[np,setNp]=useState({id:"",nombre:"",cuit:"",contacto:"",productos:"",activo:true});const[cg,setCg]=useState(false);
  ...
  if(modo==="nuevo")return<div className="min-h-screen bg-gray-50 max-w-lg mx-auto pb-24">
    ...
    <div className="p-4 flex flex-col gap-3"><Txt label="Nombre" value={np.nombre} onChange={v=>setNp(p=>({...p,nombre:v}))} ph="Razón social"/><Txt label="CUIT" value={np.cuit} onChange={v=>setNp(p=>({...p,cuit:v}))} ph="XX-XXXXXXXX-X"/><Txt label="Contacto" value={np.contacto} onChange={v=>setNp(p=>({...p,contacto:v}))} ph="Tel / email"/><TA label="Productos habituales" value={np.productos} onChange={v=>setNp(p=>({...p,productos:v}))} ph="Harina 0000, Margarina Tapera, Grasa…"/><button onClick={guardar} className="h-11 rounded-xl bg-blue-500 text-white font-semibold text-sm">Guardar proveedor</button></div>
  </div>;
  return<div className="min-h-screen bg-gray-50 max-w-lg mx-auto pb-24">
    <div className="bg-white border-b border-gray-100 px-4 pt-4 pb-3 flex items-center justify-between"><div className="flex items-center gap-3"><button onClick={onBack} className="text-gray-400 p-1">←</button><p className="font-bold text-gray-800">BD Proveedores</p></div><button onClick={()=>setModo("nuevo")} className="text-xs bg-blue-500 text-white px-3 py-1.5 rounded-lg font-medium">+ Nuevo</button></div>
    <div className="p-4 flex flex-col gap-2">{cg?<div className="flex justify-center p-8"><Spin/></div>:provs.length===0?<div className="text-center p-8 text-gray-400 text-sm">Sin proveedores registrados</div>:provs.map(p=><div key={p.id} className="bg-white border border-gray-200 rounded-xl p-3"><div className="flex items-start justify-between gap-2"><div><p className="text-sm font-semibold text-gray-800">{p.nombre}</p><p className="text-xs text-gray-400 mt-0.5">CUIT: {p.cuit}</p>{p.productos&&<p className="text-xs text-gray-500 mt-1">Productos: {p.productos}</p>}</div><button onClick={()=>eliminar(p.id)} className="text-[10px] text-red-400 hover:text-red-600 flex-shrink-0 mt-0.5">Eliminar</button></div></div>)}</div>
  </div>;
}

// ── TRAZABILIDAD AMASIJOS ─────────────────────────────────────
const ETAPAS_LABEL:Record<string,string>={
  amasado:"🫱 Amasado → Cámara",laminado:"📋 Laminado",cortadora:"✂️ Cortadora",envasado:"📦 Envasado",
};
function BTrazAmasijos({semDias,onBack}:{semDias:DiaI[];onBack:()=>void}){
  const[ams,setAms]=useState<TrazAmasijo[]>([]);const[cg,setCg]=useState(false);const[exp,setExp]=useState<string|null>(null);const[busq,sBusq]=useState("");
  const fechas=semDias.filter(d=>d.fecha).map(d=>d.fecha).sort();const desde=fechas[0]||"";const hasta=fechas[fechas.length-1]||"";
  useEffect(()=>{if(!desde)return;setCg(true);loadTrazAmasijosSemana(desde,hasta).then(c=>{setAms(c);setCg(false);});},[desde]);
  const filtrados=ams.filter(c=>{const q=busq.toLowerCase();return!q||c.num_tacho.toLowerCase().includes(q)||c.variedad?.toLowerCase().includes(q)||c.lote_amasijo?.toLowerCase().includes(q);});
  return<div className="min-h-screen bg-gray-50 max-w-lg mx-auto pb-24">
    <div className="bg-white border-b border-gray-100 px-4 pt-4 pb-3 sticky top-0 z-10">
      <div className="flex items-center gap-3 mb-3"><button onClick={onBack} className="text-gray-400 p-1 text-lg">←</button><div className="flex-1"><p className="text-base font-bold text-gray-800">🚛 Trazabilidad de Amasijos</p><p className="text-xs text-gray-400">{desde?`${fd(desde)} — ${fd(hasta)}`:""} · {ams.length} amasijos</p></div>{cg&&<Spin/>}</div>
      <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">🔍</span><input value={busq} onChange={e=>sBusq(e.target.value)} placeholder="Buscar N° tacho, variedad, lote…" className="w-full h-10 rounded-xl border border-gray-200 pl-8 pr-3 text-sm bg-white"/>{busq&&<button onClick={()=>sBusq("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs">✕</button>}</div>
    </div>
    <div className="p-4 flex flex-col gap-3">{filtrados.length===0?<div className="text-center py-12 text-gray-400"><div className="text-3xl mb-2">🚛</div><p className="text-sm">Sin amasijos registrados</p></div>
    :filtrados.map(c=>{const isExp=exp===c.num_tacho;return<div key={c.num_tacho} className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
      <div className="p-3 flex items-center justify-between cursor-pointer" onClick={()=>setExp(isExp?null:c.num_tacho)}>
        <div className="flex items-center gap-3">
          ...
          <div><div className="text-sm font-semibold text-gray-800">{c.variedad==="criollas"?"🫓":c.variedad==="integrales"?"🌾":"🟡"} {PARAMS[c.variedad as keyof typeof PARAMS]?.label||c.variedad} · {c.lote_amasijo}</div><div className="text-[10px] text-gray-400 mt-0.5">{c.etapas.length} etapas · {c.ultimo_update?new Date(c.ultimo_update).toLocaleString("es-AR",{dateStyle:"short",timeStyle:"short"}):""}</div></div>
        </div>
        <div className="flex items-center gap-2"><div className="flex gap-0.5">{(["amasado","laminado","cortadora","envasado"] as const).map(e=><div key={e} className={cn("w-2 h-2 rounded-full",c.etapas.some(x=>x.etapa===e)?"bg-green-500":"bg-gray-200")}/>)}</div><span className="text-gray-400 text-xs">{isExp?"▲":"▼"}</span></div>
      </div>
      {isExp&&<div className="border-t border-gray-100 p-3 flex flex-col gap-2">{c.etapas.sort((a,b)=>a.fecha.localeCompare(b.fecha)||a.hora.localeCompare(b.hora)).map((e,i)=><div key={i} className="bg-gray-50 rounded-xl p-2.5"><div className="flex items-center justify-between mb-1.5"><span className="text-xs font-semibold text-gray-700">{ETAPAS_LABEL[e.etapa]||e.etapa}</span><span className="text-[10px] text-gray-400">{fd(e.fecha)} {e.hora} · {e.turno} · {e.operario}</span></div><div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-[10px] text-gray-600">{Object.entries(e.datos).filter(([,v])=>v).map(([k,v])=><span key={k}><b>{k.replace(/_/g," ")}</b>: {v}</span>)}</div></div>)}</div>}
    </div>;})}
    </div>
  </div>;
}
...
function FAuditoria({u,onSave,onCancel}:{u:Usuario;onSave:(a:RAuditoria)=>void;onCancel:()=>void}){
  ...
  return<FW titulo="📋 Auditoría Interna" sub="% cumplimiento por sector" onCancel={onCancel} onSave={sv} g={g} ch={<>
    ...
    {total>0&&<div className={cn("rounded-2xl border p-4 text-center",bgTotal)}><div className="text-xs text-gray-500 mb-1">Valor de Auditoría</div><div className={cn("text-4xl font-bold",colorTotal)}>{total}%</div><div className="text-xs mt-1">{total>=90?"✓ Excelente":total>=70?"⚠ Requiere mejoras":"🔴 Crítico — acción inmediata"}</div><div className="h-3 bg-gray-200 rounded-full mt-2 overflow-hidden"><div className={cn("h-full rounded-full",total>=90?"bg-green-500":total>=70?"bg-amber-500":"bg-red-500")} style={{width:`${total}%`}}/></div></div>}
    <div className="flex flex-col gap-3">{SECTORES_AUDIT.map(s=>{const v=parseFloat(d[s.k]);const ok=!isNaN(v)&&v>=0&&v<=100;const color=ok?(v>=90?"text-green-600":v>=70?"text-amber-600":"text-red-600"):"text-gray-400";return<div key={s.k} className="flex items-center gap-3"><label className="text-sm text-gray-700 w-32 flex-shrink-0">{s.l}</label><div className="flex-1 relative"><input type="number" min="0" max="100" inputMode="decimal" value={d[s.k]} onChange={e=>sD(p=>({...p,[s.k]:e.target.value}))} placeholder="0–100" className="w-full h-10 rounded-lg border border-gray-200 px-3 pr-8 text-sm font-mono"/><span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-gray-400">%</span></div>{ok&&<div className={cn("text-sm font-bold w-12 text-right",color)}>{v}%</div>}{ok&&<div className="w-16 h-2 bg-gray-100 rounded-full overflow-hidden"><div className={cn("h-full rounded-full",v>=90?"bg-green-500":v>=70?"bg-amber-400":"bg-red-500")} style={{width:`${v}%`}}/></div>}</div>;})}</div>
...
function KPIAuditoria({auditorias}:{auditorias:RAuditoria[]}){
  ...
  const ult=auditorias[0];const color=ult.pct_total>=90?"text-green-600":ult.pct_total>=70?"text-amber-600":"text-red-600";
  ...
  return<div className={cn("rounded-2xl border p-4",bg)}>
    <div className="flex items-center justify-between mb-3"><div><div className="text-xs font-bold text-gray-600 uppercase tracking-wide">📋 Auditoría Interna</div><div className="text-[10px] text-gray-400">{fd(ult.fecha)} · {ult.responsable}</div></div><div className={cn("text-3xl font-bold",color)}>{ult.pct_total}%</div></div>
...
function RegCard({r,onDelete,isC,nota,onNota}:{r:Reg;onDelete?:()=>void;isC:boolean;nota:string;onNota:(v:string)=>void}){
  ...
  function det(){
    if(r.tipo==="temperaturas"){const rt=r as RTemp;return<div className="text-xs mt-2 grid grid-cols-2 gap-x-3 gap-y-0.5">{rt.t_camara_masas&&<span>Cám.masas: <b>{rt.t_camara_masas}°C</b></span>}{rt.t_ambiente&&<span>Ambiente: <b>{rt.t_ambiente}°C</b></span>}{rt.t_camara_pt&&<span>Cám.PT: <b>{rt.t_camara_pt}°C</b></span>}</div>;}
    if(r.tipo==="tapas"){const tp=r as RTapas;const pp=PARAMS[tp.variedad as keyof typeof PARAMS];return<div className="text-xs mt-2"><div className="font-semibold text-amber-700">{pp?.label||tp.variedad} — Lote: {tp.lote_amasijo}</div><div className="grid grid-cols-2 gap-x-3 gap-y-0.5 mt-1">{tp.prom_peso>0&&<span>Prom.peso: <b className={Math.abs(tp.prom_peso-(pp?.pesoTapa||48))>(pp?.pesoTol||2)?"text-red-600":""}>{tp.prom_peso}g</b></span>}{tp.t_masa_salida&&<span>T° masa: <b>{tp.t_masa_salida}°C</b></span>}{tp.t_agua_chiller&&<span>T° agua: <b>{tp.t_agua_chiller}°C</b></span>}{tp.recupero_cajones&&<span>Recupero: <b>{tp.recupero_cajones} cajones</b></span>}</div></div>;}
    if(r.tipo==="bpm"){const b=r as RBPM;return<div className="text-xs mt-2"><p className="text-red-700 font-medium">Operario: {b.operario} | Sector: {b.sector}</p><p className="text-gray-600">{b.incumplimientos.join(", ")}</p>{b.accion_tomada&&<p className="text-green-700 mt-0.5">Acción: {b.accion_tomada}</p>}</div>;}
  ...
  return<div className={cn("bg-white rounded-xl border p-3",al>0?"border-red-200":"border-gray-200")}>
    ...
    {exp&&<><div className="mt-2">{det()}</div>{isC&&<div className="mt-2"><input value={nota} onChange={e=>onNota(e.target.value)} placeholder="Nota calidad…" className="w-full h-8 rounded-lg border border-gray-200 px-2 text-xs bg-gray-50"/></div>}</>}
  </div>;
}
...
function ResumenPanel({registros,titulo,isCalidad,notas,onNota,eliminados,onElim,onRestore}:{registros:Reg[];titulo:string;isCalidad:boolean;notas:Record<string,string>;onNota:(id:string,v:string)=>void;eliminados:Set<string>;onElim:(id:string)=>void;onRestore:(id:string)=>void;}){
  const vis=registros.filter(r=>!eliminados.has(r.id));const k=kpis(vis);const als=extraerAlertas(vis);const rein=calcReincidencias(vis);
  function exportar(){dlTxt(buildTxt(registros,titulo,notas,eliminados),`CV_Tapas_${titulo.replace(/\s/g,"_")}.txt`);}
  return<div className="flex flex-col gap-4">
    <div className="grid grid-cols-4 gap-2">{[{l:"Registros",v:k.total,c:"gray"},{l:"Alertas",v:k.alertas,c:"red"},{l:"NC",v:k.nc,c:"amber"},{l:"Tapas",v:k.tapas,c:"green"}].map(({l,v,c})=><div key={l} className={cn("rounded-xl p-2.5 text-center border",c==="red"&&v>0?"bg-red-50 border-red-200":"bg-white border-gray-200")}><div className={cn("text-xl font-bold",c==="red"&&v>0?"text-red-600":"text-gray-800")}>{v}</div><div className="text-[10px] text-gray-400 mt-0.5">{l}</div></div>)}</div>
    {als.length>0&&<div className="bg-red-50 border border-red-200 rounded-xl p-3"><div className="text-xs font-bold text-red-700 mb-2">⚠ Alertas ({als.length})</div>{als.map((a,i)=><div key={i} className="text-xs text-red-600 py-0.5 border-b border-red-100 last:border-0">[{a.registro.hora}] {a.tipo} — Límite: {a.limite}</div>)}</div>}
    {rein.length>0&&<div className="bg-amber-50 border border-amber-200 rounded-xl p-3"><div className="text-xs font-bold text-amber-700 mb-2">🔁 Reincidencias</div>{rein.map((r,i)=><div key={i} className={cn("text-xs py-0.5",r.critico?"text-red-700 font-bold":"text-amber-700")}>{r.critico?"🔴":"🟡"} {r.tipo}: {r.count} veces</div>)}</div>}
    <div className="flex flex-col gap-2">{vis.sort((a,b)=>b.timestamp.localeCompare(a.timestamp)).map(r=><RegCard key={r.id} r={r} isC={isCalidad} onDelete={isCalidad?()=>onElim(r.id):undefined} nota={notas[r.id]||""} onNota={v=>onNota(r.id,v)}/>)}</div>
    {eliminados.size>0&&isCalidad&&<div className="mt-2 p-3 bg-gray-50 rounded-xl border border-gray-200"><div className="text-xs text-gray-500 mb-2">Eliminados de este reporte ({eliminados.size})</div>{[...eliminados].map(id=>{const r=registros.find(x=>x.id===id);if(!r)return null;const m=MODS.find(x=>x.id===r.tipo);return<button key={id} onClick={()=>onRestore(id)} className="text-xs text-blue-500 underline mr-2">{m?.icon} {r.hora} restaurar</button>;})}</div>}
...
function Dash({registros,label}:{registros:Reg[];label:string}){
  ...
  const tapData=registros.filter(r=>r.tipo==="tapas").map(r=>r as RTapas).filter(r=>r.prom_peso>0).map(r=>({hora:r.hora,peso:r.prom_peso,variedad:r.variedad,ok:Math.abs(r.prom_peso-(PARAMS[r.variedad as keyof typeof PARAMS]?.pesoTapa||48))<=(PARAMS[r.variedad as keyof typeof PARAMS]?.pesoTol||2)}));
  const bpmCount=registros.filter(r=>r.tipo==="bpm").length;
  const ncCount=registros.filter(r=>r.tipo==="nc").length;
  return<div className="p-4 flex flex-col gap-4">
    <div className="text-xs font-bold text-gray-500 uppercase tracking-wide">{label}</div>
    <div className="grid grid-cols-2 gap-2">
      {[{l:"Controles tapas",v:k.tapas,ic:"🫓"},{l:"BPM incidencias",v:k.bpm_inc,ic:"👤"},{l:"NC generadas",v:k.nc,ic:"⚠️"},{l:"Alertas totales",v:k.alertas,ic:"🔴"}].map(({l,v,ic})=><div key={l} className="bg-white rounded-2xl border border-gray-200 p-3 flex flex-col gap-1"><div className="text-xl">{ic}</div><div className="text-2xl font-bold text-gray-800">{v}</div><div className="text-[10px] text-gray-400">{l}</div></div>)}
    </div>
    {tapData.length>0&&<div className="bg-white rounded-2xl border border-gray-200 p-4">
      <div className="text-xs font-bold text-gray-600 mb-2 uppercase tracking-wide">⚖️ Pesos de tapas (g)</div>
      <ResponsiveContainer width="100%" height={80}><BarChart data={tapData} margin={{top:0,right:0,left:-30,bottom:0}}><XAxis dataKey="hora" tick={{fontSize:8}}/><YAxis domain={[20,55]} tick={{fontSize:8}}/><Tooltip formatter={(v)=>`${v}g`}/><Bar dataKey="peso" fill="#f59e0b" radius={[3,3,0,0]} name="Peso (g)"/></BarChart></ResponsiveContainer>
    </div>}
  </div>;
}
...
function VDia({u,mes,sem,dia,onBack}:{u:Usuario;mes:MesI;sem:SemI;dia:DiaI;onBack:()=>void}){
  const[registros,sR]=useState<Reg[]>([]);const[form,sF]=useState<Tipo|"auditoria"|null>(null);const[cg,sCg]=useState(false);const[notas,sNotas]=useState<Record<string,string>>({});const[elim,sElim]=useState<Set<string>>(new Set());const[vista,sV]=useState<"registros"|"resumen">("registros");const[traz,sTraz]=useState(false);const[auditorias,sAud]=useState<RAuditoria[]>([]);
  ...
  return<div className="min-h-screen bg-gray-50 max-w-lg mx-auto pb-28">
    <div className="bg-white border-b border-gray-100 px-4 pt-4 pb-3 sticky top-0 z-10">
      <div className="flex items-center gap-3"><button onClick={onBack} className="text-gray-400 p-1">←</button><div className="flex-1"><p className="text-xs text-gray-400">{mes.label} · Sem {sem.semana}</p><p className="text-base font-bold text-gray-800">{fd(dia.fecha)}</p></div>{cg&&<Spin/>}{k.alertas>0&&<ABadge n={k.alertas}/>}</div>
      <div className="flex gap-1 mt-2 bg-gray-100 rounded-xl p-1">{([{k:"registros",l:"Registros"},{k:"resumen",l:"Resumen"}] as const).map(x=><button key={x.k} onClick={()=>sV(x.k)} className={cn("flex-1 text-xs font-medium py-1.5 rounded-lg",vista===x.k?"bg-white text-gray-800 shadow-sm":"text-gray-500")}>{x.l}</button>)}</div>
    </div>
    {vista==="registros"&&<div className="p-4">
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        ...
        ].map(x=><button key={x.id} onClick={()=>sF(x.id)} className={cn("h-14 rounded-xl text-white font-semibold text-sm flex items-center justify-center gap-2",x.col)}><span>{x.ic}</span><span>{x.l}</span></button>)}
      </div>
      ...
      {registros.length===0?<div className="text-center py-12 text-gray-400"><div className="text-4xl mb-3">🫓</div><p className="text-sm">Sin registros hoy</p><p className="text-xs mt-1">Tocá un formulario para comenzar</p></div>
    ...
    {vista==="resumen"&&<div className="px-4 pt-4"><ResumenPanel registros={registros} titulo={`${fd(dia.fecha)}`} isCalidad={isC} notas={notas} onNota={(id,v)=>sNotas(p=>({...p,[id]:v}))} eliminados={elim} onElim={id=>sElim(p=>new Set([...p,id]))} onRestore={id=>sElim(p=>{const n=new Set(p);n.delete(id);return n;})}/></div>}
  </div>;
}
  ...
  return<div className="min-h-screen bg-gray-50 max-w-lg mx-auto pb-24">
    <div className="bg-white border-b border-gray-100 px-4 pt-4 pb-3">
      <div className="flex items-center justify-between">
        <div><p className="text-xs text-gray-400">Sabores Express · Control Tapas</p><p className="text-base font-bold text-gray-800">{u.nombre}</p></div>
        <div className="flex items-center gap-2"><div className={cn("text-xs font-semibold px-2 py-1 rounded-full",u.turno==="TM"?"bg-amber-100 text-amber-700":u.turno==="TT"?"bg-blue-100 text-blue-700":"bg-indigo-100 text-indigo-700")}>{TURNOS.find(t=>t.id===u.turno)?.label}</div><button onClick={onLogout} className="text-xs text-gray-400 hover:text-gray-600">Salir</button></div>
      </div>
      <div className="flex gap-1 mt-3 text-xs text-gray-400 flex-wrap">
        <span className="bg-amber-50 text-amber-700 px-2 py-0.5 rounded-full">🫓 Criollas P251</span>
        <span className="bg-green-50 text-green-700 px-2 py-0.5 rounded-full">🌾 Integrales P250</span>
        <span className="bg-yellow-50 text-yellow-700 px-2 py-0.5 rounded-full">🟡 Pastelitos P252</span>
      </div>
    </div>
